...
// Update the clinic table with the new window number
$updateQuery = "UPDATE assets SET window = '$windowNumber' WHERE queue_number = '$queueNumber'";
$result = $conn->query($updateQuery);

// Fetch the current window number
$currentWindowQuery = "SELECT window FROM assets WHERE queue_number = '$queueNumber'";
...
if ($currentWindowResult) {
    if ($currentWindowResult->num_rows > 0) {
        ...
        error_log("Fetched Window: " . $currentWindow);

        // Insert or update the display table
        $updateDisplayQuery = "INSERT INTO display (queue_number, window, officeName) VALUES ('$queueNumber', '$currentWindow', 'Assets')

...
$query = "SELECT * FROM clinic WHERE status = 0 ORDER BY timestamp ASC LIMIT 1"; // Adjust the query as needed
...
if (!empty($clinic_data)) {
    foreach ($clinic_data as $queue_item) {
        ...
        if ($queue_item['window'] !== null && !in_array($queue_item['window'], $displayed_windows)) {
            $htmlContent .= '<div class="clinic-queue">';

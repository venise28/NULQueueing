...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $fetchDataSql = "SELECT * FROM guidance WHERE queue_number = '$queueNumber'";
    ...
    if ($result->num_rows == 1) {
        ...
        $insertSql = "INSERT INTO guidance_logs ( queue_number, student_id, endorsed_from, transaction, remarks, status, timeout, timestamp) VALUES (
            '" . $row['queue_number'] . "',
            '" . $row['student_id'] . "',
            '" . $row['endorsed_from'] . "',
            '" . $row['transaction'] . "',
        ...
        if ($conn->query($insertSql) === true) {
            ...
            $deleteSql = "DELETE FROM guidance WHERE queue_number = '$queueNumber'";

...
$query = "SELECT * FROM itso WHERE status = 0 ORDER BY timestamp ASC LIMIT 1"; // Adjust the query as needed
...
$htmlContent = '';
if (!empty($itso_data)) {
    foreach ($itso_data as $queue_item) {
        ...
        if ($queue_item['window'] !== null && !in_array($queue_item['window'], $displayed_windows)) {
            $htmlContent .= '<div class="itso-queue">';

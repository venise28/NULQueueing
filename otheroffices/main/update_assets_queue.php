...
$query = "SELECT * FROM assets WHERE status = 0 ORDER BY timestamp ASC LIMIT 1"; // Adjust the query as needed
...
if (!empty($assets_data)) {
    foreach ($assets_data as $queue_item) {
        ...
        if ($queue_item['window'] !== null && !in_array($queue_item['window'], $displayed_windows)) {
            $htmlContent .= '<div class="assets-queue">';
